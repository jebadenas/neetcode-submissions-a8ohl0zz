        ...
        for (int rowIndex = 0; rowIndex < board.length ; rowIndex++){
            ...
            for (int col = 0; col < row.length; col++){
                ...
                if (c != '.'){
                    ...
                    boolean x = columns.computeIfAbsent(col, k -> new HashSet<>()).contains(c);
                    boolean y = rows.computeIfAbsent(rowIndex, k -> new HashSet<>()).contains(c);
                    boolean z = squares.computeIfAbsent(squareKey, k -> new HashSet<>()).contains(c);
